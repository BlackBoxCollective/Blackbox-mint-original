...
import {
  contractAddr,
  contract,
} from "../../../../contracts/BoardingPassContract";
...
const Form = ({ colorInvert = false }) => {
  ...
  const WalletAddress = () => {
    ...
    if (wallet !== undefined) {
      ...
      displayWallet =
        catWallet.substring(0, 9) +
        "..." +
        catWallet.substring(catWallet.length - 9, catWallet.length);
    } else {
  ...
  const ContractAddress = () => {
    ...
    var displayAddr =
      catAddr.substring(0, 9) +
      "..." +
      catAddr.substring(catAddr.length - 9, catAddr.length);
    return (
      <Typography>
        <Link color="inherit" href={link}>
          {displayAddr}
    ...
    <Box>
      ...
      >
        <Box
          ...
          src={AccessPassLogo}
          height={{ xs: 1, md: 0.6, lg: 0.5 }}
          width={{ xs: 1, md: 0.6, lg: 0.5 }}
        />
      </Box>
      <Grid container spacing={1}>
        ...
        >
          ...
          {wallet && (
            <Web3Button fullWidth variant="contained" onClick={mint}>
              Mint {mintAmount} for {0.08 * mintAmount} ETH
            </Web3Button>
          )}{" "}
          {!wallet && (
            <Web3Button fullWidth variant="contained" onClick={() => connect()}>
              Connect Wallet
            </Web3Button>
          )}
        </Grid>
